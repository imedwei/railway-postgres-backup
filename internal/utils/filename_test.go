...
func TestGenerateBackupFilename(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:   "with prefix",
			prefix: "postgres",
			want:   "postgres-2025-01-21T10-30-45-123Z.tar.gz",
		},
		{
			name:   "without prefix",
			prefix: "",
			want:   "backup-2025-01-21T10-30-45-123Z.tar.gz",
		},
		{
		...
		},
		{
			...
			want:   "my-database-backup-2025-01-21T10-30-45-123Z.tar.gz",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := GenerateBackupFilename(tt.prefix, timestamp)
			if got != tt.want {
...
func TestParseBackupFilename(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			filename: "postgres-2025-01-21T10-30-45-123Z.tar.gz",
			want:     time.Date(2025, 1, 21, 10, 30, 45, 123000000, time.UTC),
			wantErr:  false,
		},
		{
			name:     "valid without prefix",
			filename: "backup-2025-01-21T10-30-45-123Z.tar.gz",
			want:     time.Date(2025, 1, 21, 10, 30, 45, 123000000, time.UTC),
			wantErr:  false,
		},
		{
...
func TestRoundTrip(t *testing.T) {
	...
	for _, prefix := range prefixes {
		t.Run("prefix="+prefix, func(t *testing.T) {
			...
			filename := GenerateBackupFilename(prefix, original)
...
func TestGenerateBackupFilename_Format(t *testing.T) {
	...
	timestamp := time.Now()
	filename := GenerateBackupFilename("test", timestamp)
